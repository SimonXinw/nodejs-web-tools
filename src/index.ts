...
class Application {
  ...
  async start(): Promise<void> {
    ...
    // 检查是否启动 API 服务器
    const enableApi = process.env.ENABLE_API === 'true' || process.argv.includes('--api');
...
if (args.includes('--manual') || args.includes('-m')) {
  ...
  // 查看历史数据
  const limit = parseInt(args[args.indexOf('--history') + 1] || args[args.indexOf('-h') + 1] || '10');
  app.getHistoricalData(limit).then((data) => {
